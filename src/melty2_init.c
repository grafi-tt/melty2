...
static const uint8_t code5b6b_tbl[64] = {
...
static void encode_key(const uint32_t* seed, uint32_t* key) {
    ...
    for (int i = 0; i < 8; ) {
        uint32_t s = *seed++;
        do {
            // Read 4 bits from `s` and 1 bits from `c`, then apply 5b/6b to get 6 bits.
            // Repeat the procedure 4 times and concatenate outputs to obtain 24 bits `t`.
            uint32_t t = 0;
            while (!(t & 0x3f)) {
                uint32_t n = (s & UINT32_C(0xf)) | (c & UINT32_C(1)) << 4;
                s >>= 4;
                c >>= 1;
                uint32_t code = code5b6b_tbl[2 * n + !is_positive];
                uint32_t code_alt = code5b6b_tbl[2 * n + is_positive];
                t = code << 18 | t >> 6;
